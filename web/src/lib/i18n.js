export const I18N = {
  zh: {
    ...
    models: {
      ...
      provider: n => `服务商 ${n}`, modelCount: n => `${n} 个模型`, baseMissing: '尚未填写 BaseURL', saveFailed: '保存失败', deleteProviderTitle: '删除服务商', name: '名称', protocol: '官方协议', nameExample: '例如 gpt55_medium', nameHelp: '用于区分服务商；内部协议前缀会自动维护。', tempShown: '临时显示', hiddenByDefault: '默认隐藏', keyPlaceholder: '保留掩码表示不覆盖已保存密钥', reread: '重新读取', cannotSave: '此服务商暂时不能保存', beforeSave: '保存前请留意', openProvider: '查看该服务商的连接配置', providerModels: '该服务商的模型', clearProviderModels: '清空模型', clearProviderModelsConfirm: (name, count) => `清空“${name}”的全部 ${count} 个模型？\n\n同时移除故障转移组中对此服务商的模型引用，保留服务商连接配置与密钥。此操作仅修改草稿，保存后才生效。`, providerNoModels: '这个服务商还没有模型。', providerFormIncomplete: '请填写名称和 BaseURL。',
  ...
  en: {
    ...
    models: {
      ...
      provider: n => `Provider ${n}`, modelCount: n => `${n} model${n === 1 ? '' : 's'}`, baseMissing: 'BaseURL not configured', saveFailed: 'Save failed', deleteProviderTitle: 'Delete provider', name: 'Name', protocol: 'Official protocol', nameExample: 'For example gpt55_medium', nameHelp: 'Identifies the provider; the internal protocol prefix is maintained automatically.', tempShown: 'Temporarily visible', hiddenByDefault: 'Hidden by default', keyPlaceholder: 'Keep the masked value to preserve the saved key', reread: 'Read again', cannotSave: 'This provider cannot be saved yet', beforeSave: 'Review before saving', openProvider: 'Open this provider’s connection', providerModels: 'Models from this provider', clearProviderModels: 'Clear models', clearProviderModelsConfirm: (name, count) => `Clear all ${count} models from “${name}”?\n\nReferences in failover groups will also be removed. Provider connection settings and API key are kept. This only changes the draft until you save.`, providerNoModels: 'This provider has no models yet.', providerFormIncomplete: 'Enter a name and a BaseURL.',
