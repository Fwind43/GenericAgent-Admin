...
export function createChatSessionCache({ maxBytes = 64 * 1024 * 1024, maxEntries = 8 } = {}) {
  ...
  const clear = () => { generation++; pending.clear(); entries.clear(); bytes = 0 }
  const load = async (url, { fetcher = fetch, signal } = {}) => {
    ...
    try {
      ...
      const checkCurrent = () => {
        if (signal?.aborted || epoch !== generation || pending.get(url) !== token) throw new DOMException('Session request superseded', 'AbortError')
      }
      checkCurrent()
      ...
      if (pending.get(url) === token) pending.delete(url)
    }
  }
  return { load, clear }
}
