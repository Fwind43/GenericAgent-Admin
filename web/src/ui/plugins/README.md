...
- `chat.navigation` and `chat.followToolbar`: declarative `content` replaces the sidebar header navigation and follow-latest toolbar, respectively. Missing or invalid declarations retain the corresponding host slot; replacement trees must contain `newChat` and `collapseSidebar` for navigation and `followLatest` for the toolbar, otherwise they are rejected on import or fall back independently at runtime. The message list, composer and sidebar business siblings are not replaced. Navigation summaries distinguish busy, session-management and ready states and expose a nonnegative session count. Follow summaries distinguish loading, busy, latest messages available and already at latest; these use the same host state as action guards. The 1.3.0 sample displays these summaries.
