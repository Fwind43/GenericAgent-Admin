...
export function PluginPreview({ bundle }) {
  ...
  return <section className="gaui-preview" aria-label="Fictional plugin preview"><p>Fictional data only. Preview does not enable or save.</p>{bundle.manifest.surfaces.map(area => <section key={area}><h3>{area}</h3><ExternalView bundle={bundle} area={area} data={FICTION} invoke={n => setAction('Simulated: ' + n.action)}/>{['chat.sidebar', 'chat.messages', 'chat.composer'].includes(area) && <div data-gaui-density={bundle.config.density || bundle.views[area].layout.density}><p>Host-owned example {area.slice(5)} slot</p>{area === 'chat.composer' && <textarea aria-label="Fictional draft" defaultValue="Example unsent draft"/>}</div>}</section>)}<p role="status">{action}</p></section>
}
export function PluginManager() {
  ...
  return <section id="general-plugins" className="gaui-manager" aria-label="UI plugins"><h2>UI plugins / UI插件</h2><p><a href="/ui-plugins/local-workshop.gaui.zip" download>Download example package</a></p><p>Local declarative packages only: no JS/React, network access or account sync.</p><p>Active: {ui?.bundle?.manifest.name || 'Built-in default'}</p>
    ...
    {candidate && <><h3>Coverage</h3><p>Provided: {candidate.manifest.surfaces.join(', ')}</p><p>Default fallback: {AREAS.filter(a => !candidate.manifest.surfaces.includes(a)).join(', ') || 'None among these seven areas'}. All other admin pages, chat dialogs, message content renderers, sensitive fields and confirmation flows remain host-owned.</p><p>Chat packages control declarative headings, summary badges, navigation actions and region layout. Message rendering, input, attachments and confirmations remain stable host controls.</p>
