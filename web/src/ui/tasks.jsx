...
import '../pages/tasks-workbench.css'
import './tasks.css'
...
function TasksView({ model, actions, studio = false }) {
  const { t, lang, section, schedule, taskSvcs, reflectSvcs, llms, actionStates, goals, autonomousReports, busy, tasks, selectedTask, runningGoals, serviceId, runtimeId } = model
  const { onSection, onStart, onStop, onLogs, onAutostart, onServiceModel, onReflectStart, onRefreshGoals, onOpenGoal, setServiceId, setRuntimeId } = actions
  const scheduleState = { ...model.editor, ...actions.schedule }
  const serviceIndex = (items, selected, select) => studio ? <div className="tasks-service-cards">{items.map(svc => <ServiceRow key={svc.name} svc={svc} t={t} llms={llms} actionState={actionStates[svc.name]} onStart={onStart} onStop={onStop} onLogs={onLogs} onAutostart={onAutostart} onModel={onServiceModel} onReflectStart={onReflectStart}/>)}</div> : <div className="service-master-detail">
  ...
  return <section data-tasks-layout={studio ? 'studio' : 'default'} className={`tasks-page tasks-desktop-workbench ${studio ? 'tasks-studio' : 'tasks-default'}`}>
    <header className="tasks-navigation">{studio && <h2>{lang === 'zh' ? '任务工作台' : 'Task desk'}</h2>}
    ...
    </header><div className="tasks-content">
      ...
      <Panel title={t.lists.scheduledTasks}>
        ...
        <div className="task-list clean-list" aria-busy={scheduleState.loading}>
          {scheduleState.loading
            ...
            : tasks.length
              ? tasks.map(task => studio ? <button type="button" className="tasks-schedule-card" key={task.id} disabled={busy} aria-pressed={task.id === scheduleState.taskId} onClick={() => scheduleState.loadTask(task.id)}><strong>{task.id}</strong><span>{task.schedule} / {task.repeat}</span><small>{task.enabled ? t.enabled : t.disabled}</small><p>{task.prompt}</p></button> : <TaskRow key={task.id} task={task} t={t} selected={task.id === scheduleState.taskId} disabled={busy} onEdit={scheduleState.loadTask}/>)
              : <p className="muted">{t.hints.noTasks}</p>}
        </div>
      </Panel>
...
export function DefaultTasks(props) { return <TasksView {...props}/> }
export function StudioTasks(props) { return <TasksView {...props} studio/> }
