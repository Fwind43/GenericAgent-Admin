...
it('registers independent appearance/remote DOM, whitelist models and preserves both drafts across packages', async () => {
  ...
  group('appearance')
  fireEvent.click(section('appearance').getByRole('button', { name: 'Edit colors' }))
  change('theme-color-accent', '#abcdef')
  group('remote')
  ...
  fireEvent.click(section('appearance').getByRole('button', { name: 'Edit colors' }))
  change('theme-color-accent', '#fedcba')
  await select('default')
  expect(section('appearance').getByLabelText('accent', { exact: true }).value).toBe('#fedcba')
  await select('studio')
  expect(section('appearance').getByLabelText('accent', { exact: true }).value).toBe('#fedcba')
...
it('routes theme/language and color validation, failure, retry, cancel and reset through the host', async () => {
  ...
  fireEvent.click(section('appearance').getByRole('button', { name: 'Edit colors' }))
  change('theme-color-accent', 'not-a-color')
  expect(section('appearance').getByRole('button', { name: 'Save colors' }).disabled).toBe(true)
  ...
  fireEvent.click(section('appearance').getByRole('button', { name: 'Save colors' }))
  await waitFor(() => expect(section('appearance').getByRole('alert').textContent).toContain('Draft retained'))
  expect(section('appearance').getByLabelText('accent', { exact: true }).value).toBe('#abcdef')
  fireEvent.click(section('appearance').getByRole('button', { name: 'Save colors' }))
  await waitFor(() => expect(section('appearance').getByText('Colors saved.')).toBeTruthy())
  expect(persistCustomColors).toHaveBeenLastCalledWith({ accent: '#abcdef' }, 'dark')
  fireEvent.click(section('appearance').getByRole('button', { name: 'Edit colors' }))
  fireEvent.click(section('appearance').getByRole('button', { name: 'Restore defaults' }))
  expect(captured.appearance.model.colors.draft).toEqual({})
  fireEvent.click(section('appearance').getByRole('button', { name: 'Cancel preview' }))
...
it('retains color save pending state and rejects duplicate actions across a package switch', async () => {
  ...
  await select('default')
  expect(section('appearance').getByRole('button', { name: 'Saving…' }).disabled).toBe(true)
  await act(async () => { resolve({ accent: '#abcdef' }); await pending })
  expect(section('appearance').getByText('Colors saved.')).toBeTruthy()
})
