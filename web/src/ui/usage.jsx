    ...
    {!model.error && model.hasData && <>
      ...
      {model.empty ? <div className="usage-state">{c.empty}</div> : <>
        ...
        <section id="usage-models" className="usage-panel">
          ...
          <div className="usage-table-wrap" tabIndex={0} role="region" aria-label={c.models}><table><thead><tr><th>{c.model}</th><th>{c.input}</th><th>{c.output}</th><th>{c.cacheRead}</th><th>{c.cacheWrite}</th><th>{c.total}</th></tr></thead><tbody>
            {model.rows.map(row => <tr key={row.id}><td><strong>{row.name}</strong><small>{row.id}</small></td><td><TokenValue value={row.input}/></td><td><TokenValue value={row.output}/></td><td><TokenValue value={row.cacheRead}/></td><td><TokenValue value={row.cacheWrite}/></td><td><b><TokenValue value={row.total}/></b></td></tr>)}
            {!model.rows.length && <tr><td colSpan={6}>{c.noMatches}</td></tr>}
          </tbody></table></div>
        </section>
      </>}
    </>}
          ...
          {model.empty ? <div className="usage-state">{c.empty}</div> : <>
            <section id="usage-models" className="studio-usage-models">
              ...
              <ul className="studio-usage-model-list" aria-label={c.models}>{model.rows.map(row => <li key={row.id}>
                ...
                <dl><div><dt>{c.input}</dt><dd><TokenValue value={row.input}/></dd></div><div><dt>{c.output}</dt><dd><TokenValue value={row.output}/></dd></div><div><dt>{c.cacheRead}</dt><dd><TokenValue value={row.cacheRead}/></dd></div><div><dt>{c.cacheWrite}</dt><dd><TokenValue value={row.cacheWrite}/></dd></div></dl>
              </li>)}</ul>
