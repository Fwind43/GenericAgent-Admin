...
export default function ThemeColorEditor({ theme, lang = 'en', active = true }) {
  const zh = lang === 'zh'
  ...
  return <section className="theme-editor" aria-label={zh ? '自定义主题颜色' : 'Custom theme colors'}>
    <header><div><h3>{zh ? '自定义主题颜色' : 'Custom theme colors'}</h3>
      ...
      {!open && <button type="button" onClick={begin}>{zh ? '编辑颜色' : 'Edit colors'}</button>}
    </header>
    {open && <>
          ...
          {tokens.map(token => {
            ...
            return <div className="theme-color-field" key={token}>
              ...
              <div className="theme-color-inputs">
                <input type="color" aria-label={`${token} color picker`} value={/^#[\da-f]{6}$/i.test(value) ? value : '#808080'} onChange={e => update({ ...draft, [token]: e.target.value })}/>
                <input id={`theme-color-${token}`} value={value} maxLength={64} placeholder={zh ? '继承预设' : 'Inherit preset'} aria-invalid={bad} aria-describedby={bad ? `theme-error-${token}` : undefined} onChange={e => update({ ...draft, [token]: e.target.value })}/>
              </div>
      ...
      <div className="theme-editor-actions">
        <button type="button" disabled={busy} onClick={() => update({})}>{zh ? '恢复默认（草稿）' : 'Restore defaults (draft)'}</button>
        <span/>
        <button type="button" disabled={busy} onClick={cancel}>{zh ? '取消预览' : 'Cancel preview'}</button>
        <button type="button" className="primary" disabled={busy || invalid.length > 0} onClick={save}>{busy ? (zh ? '保存中…' : 'Saving…') : (zh ? '保存颜色' : 'Save colors')}</button>
      </div>
    </>}
