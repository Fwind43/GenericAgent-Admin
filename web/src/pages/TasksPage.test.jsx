...
afterEach(cleanup)
it('organizes runtime reports into a selectable list and in-place detail without changing its action', async () => {
 ...
 const { container } = render(<TasksPage t={I18N.en} section="runs" llms={[]} schedule={{ tasks: [] }} taskSvcs={[]} goals={[]} reflectSvcs={[]} autonomousReports={[report]} scheduleState={{ artifactTitle: report.path, artifact: 'Fixture body', readArtifact }} />)
 fireEvent.click(await screen.findByRole('button', { name: /Fixture report/ }))
...
it('announces failure without discarding the editable draft', () => {
 const noop=vi.fn()
 render(<TasksPage t={I18N.en} section="scheduled" llms={[]} autonomousReports={[]} schedule={{tasks:[]}} taskSvcs={[]} reflectSvcs={[]} goals={[]} scheduleState={{error:'Fixture failure',loading:false,taskId:'fixture',editorMode:'json',editor:'draft remains',setEditor:noop,reload:noop,createTask:noop,newTaskId:'',setNewTaskId:noop,saveTask:noop,deleteTask:noop}} />)
 ...
 expect(screen.getByRole('button',{name:I18N.en.remove}).className).toContain('danger')
})
